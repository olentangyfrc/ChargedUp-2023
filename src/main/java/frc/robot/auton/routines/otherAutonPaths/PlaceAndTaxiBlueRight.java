...
import frc.robot.auton.commands.TaxiBlueRight;
...
public class PlaceAndTaxiBlueRight extends SequentialCommandGroup {
  /** Creates a new BottomTaxi. */
  public PlaceAndTaxiBlueRight(ActiveIntake intake, SwerveDrivetrain drivetrain, Claw claw, ClawPitch clawPitch, Elevator elevator, AutonPaths paths) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
        new ScoreCubeHigh(elevator, claw, clawPitch, intake),
        new PlaceCube(elevator, claw, clawPitch, intake),
        new TaxiBlueRight(drivetrain)
    );
  }
}
